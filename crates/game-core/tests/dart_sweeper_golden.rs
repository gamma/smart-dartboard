...
#[test]
fn dart_sweeper_matches_shared_seeded_golden_fixture() {
    let fixture: Fixture =
        serde_json::from_str(include_str!("../../../fixtures/games/dart_sweeper_v2.json"))
            .expect("fixture");
...
fn snapshot(state: &sdb_game_core::RegisteredGameState) -> Value {
    let mut revealed = state.mode_state["revealed"]
        .as_object()
        .expect("revealed")
        .iter()
        .map(|(field, count)| serde_json::json!([field.parse::<u8>().expect("field"), count]))
        .collect::<Vec<_>>();
    revealed.sort_by_key(|item| item[0].as_u64());
    serde_json::json!({
        "scores": state.players.iter().map(|player| player.score).collect::<Vec<_>>(),
        "seeded": state.mode_state["seeded"], "direct_hit_seen": state.mode_state["direct_hit_seen"],
        "mines": state.mode_state["mines"], "revealed": revealed, "exploded": state.mode_state["exploded"],
        "lives": state.mode_state["lives"], "last_effect": state.mode_state["last_effect"],
        "effect_points": state.mode_state["effect_points"], "effect_field": state.mode_state["effect_field"],
        "revealed_count": state.mode_state["revealed_count"], "random_cursor": state.random_cursor,
