...
#[test]
fn avoid_bomb_matches_shared_seeded_golden_fixture() {
    let fixture: Fixture =
        serde_json::from_str(include_str!("../../../fixtures/games/avoid_bomb_v4.json"))
            .expect("fixture");
    assert_eq!(fixture.schema_version, 1);
    assert_eq!(fixture.ruleset_version, 4);
    ...
    for step in fixture.steps {
        ...
        let bomb_ids = state.mode_state["bombs"]
            .as_array()
            .expect("bombs")
            .iter()
            .map(|bomb| format!("{}:{}", bomb["ring"].as_str().expect("ring"), bomb["field"]))
            .collect::<Vec<_>>();
